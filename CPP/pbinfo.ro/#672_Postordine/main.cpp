...
void printTreePostoeder(std::vector<node> binaryTree, int currentIndex);

int main() {

    // Reading binary tree
    in = fopen("postordine.in", "r");
    fscanf(in, "%d", &n);
    for(int i = 1; i <= n; ++i) {
        node tmp;
        tmp.index = i;
    ...
    out = fopen("postordine.out", "w");
    printTreePostoeder(biTree, rootIndex);
    fclose(out);
...
void printTreePostoeder(std::vector<node> binaryTree, int currentIndex) {
    if(binaryTree[currentIndex -1].value) {
        printTreePostoeder(binaryTree, binaryTree[currentIndex -1].left);
        printTreePostoeder(binaryTree, binaryTree[currentIndex -1].right);
